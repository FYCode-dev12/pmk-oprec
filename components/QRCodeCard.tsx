...
export function QRCodeCard({ url, title, hideLink, hideIcon }: { url: string; title: string, hideLink?: boolean, hideIcon?: boolean }) {
    const handleCopy = () => {
    ...
    return (
        <div className="flex flex-col items-center bg-card p-6 rounded-2xl border-2 border-accent shadow-md max-w-sm w-full mx-auto">
            {!hideIcon && <h3 className="font-serif font-bold text-foreground mb-4 text-center">{title}</h3>}
            {hideIcon && <h3 className="font-serif font-bold text-foreground mb-4 text-center">{title}</h3>}
